...
for input in ${inputs}; do
	case ${bench} in
		...
		bst)
			...
			;;
		ep)
			args="-w 0 -c -m 16 -x 128 -q"
			;;
		fir)	 
			args="-w 0 -y 64 -x 750 -q"
#			args="-w 0 -y 4 -x 3400 -q"
			;;
		ga)
		...
		hist)
			...
			;;
		kmeans)
			args="-w 0 -i ${input_dir}/10000_34.txt -q" 
			;;
		knn)
			args="-w 0 -i ${input_dir}/knn_input16.txt -q"
			;;
		pr)
			args="-w 0 -m 16 -i ${input_dir}/$input -q"
			;;
		*)
			args="-w 0 -i ${input_dir}/$input -q"
			;;
